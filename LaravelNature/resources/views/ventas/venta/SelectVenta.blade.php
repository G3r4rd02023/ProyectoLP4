...
@section('content_header')
    <h1>Venta</h1>
    <a href="crearFormVe" class="btn btn-dark">Crear</a>
@stop
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table id="miTabla" class="table table-bordered table-hover">
                        ...
                        <tbody>
                            @foreach($ResulVenta as $venta)
                                <tr class="table-{{ $loop->even ? 'success' : 'warning' }} product-row">
                                    <td>{{ $venta['COD_VENTA'] }}</td>
                                    <td>{{ $venta['FEC_AGREGADO'] }}</td>
                                    <td>{{ $venta['DET_DETALLE_VEN'] }}</td>
                                    <td>{{ $venta['RTN_IDENTIFICACION'] }}</td>
                                    <td>{{ $venta['NOM_ENTIDAD'] }}</td>

                                    <td class="d-none d-sm-table-cell">                           
                                        <a href="/UpdateFormVe/{{ $venta['COD_VENTA'] }}" class="btn btn-info editar-btn">Editar</a>
                                        <form method="POST" action="/EliminarVenta/{{ $venta['COD_VENTA'] }}" style="display: inline;">
                                            @method('DELETE')
                                            @csrf
