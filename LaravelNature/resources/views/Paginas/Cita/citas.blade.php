            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-bordered table-hover">
                        ...
                        <tbody>
                            @foreach($ResulCita as $cita)
                                <tr class="table-{{ $loop->even ? 'success' : 'warning' }} cita-row">
                                    <td>{{ $cita['COD_CITA'] }}</td>
                                    <td>{{ $cita['COD_SERVICIO'] }}</td>
                                    <td>{{ $cita['EST_CITA'] }}</td>
                                    <td>{{ $cita['DUR_CITA'] }}</td>
                                    <td>{{ $cita['FEC_CITA'] }}</td>
                                    <td>{{ $cita['HOR_CITA'] }}</td>
                                    <td>{{ $cita['FEC_RECORDATORIO'] }}</td>
                                    <td class="d-none d-sm-table-cell">
                                    <a href="/UpdateForm/{{ $cita['COD_CITA'] }}" class="btn btn-info editar-btn">Editar</a>
                                        <form method="POST" action="/EliminarCita/{{ $cita['COD_CITA'] }}" style="display: inline;">
                                            @method('DELETE')
                                            @csrf
