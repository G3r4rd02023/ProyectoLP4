...
@section('content_header')
    <h1>Pagos</h1>
    <a href="crearFormPa" class="btn btn-dark">Crear</a>
@stop
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-bordered table-hover">
                        ...
                        <tbody>
                            @foreach($ResulPago as $pago)
                                <tr class="table-{{ $loop->even ? 'success' : 'warning' }} pago-row">
                                    <td>{{ $pago['COD_PAGO'] }}</td>
                                    <td>{{ $pago['COD_SERVICIO'] }}</td>
                                    <td>{{ $pago['TIP_PAGO'] }}</td>
                                    <td>{{ $pago['PLA_PAGO'] }}</td>
                                    <td class="d-none d-sm-table-cell">
                                    <a href="/UpdateFormPa/{{ $pago['COD_PAGO'] }}" class="btn btn-info editar-btn">Editar</a>
                                        <form method="POST" action="/EliminarPago/{{ $pago['COD_PAGO'] }}" style="display: inline;">
                                            @method('DELETE')
                                            @csrf
